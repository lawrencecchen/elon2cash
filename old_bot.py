...
from db import init_db, db_buy, db_sell, get_portfolio, get_current_holdings
...
async def on_message(message):
    ...
    if message.content.startswith("~clock"):
        ...
        await message.channel.send(
            "The market is {}".format("open." if clock.is_open else "closed.")
        )
    ...
    if message.content.startswith("$"):
        ...
        try:
            await message.channel.send(
                "{} is currently trading at ${:,.2f}".format(
    ...
    if message.content.startswith("!buy"):
        ...
        try:
            ...
            db_buy(message.author, symbol.upper(), shares, price)
            await message.channel.send(
                "{} bought {:,.2f} shares of {} for ${:,.2f}.".format(
                    str(message.author), shares, symbol.upper(), float(price)
                )
            )
        except:
            await message.channel.send(
                "Could not buy for {}.".format(str(message.author))
            )
    ...
    if message.content.startswith("!sell"):
        ...
        try:
            ...
            db_sell(message.author, symbol.upper(), shares, price)
            await message.channel.send(
                "{} sold ${:,.2f} of {}.".format(
                    str(message.author), float(price), symbol.upper()
                )
            )
        except:
            await message.channel.send(
                "Could not sell for {}.".format(str(message.author))
            )
        ...
        try:
            ...
            await message.channel.send(
                """{}'s owns {} shares of {}""".format(
                    message.author, current["qty"], symbol.upper()
                )
            )
        except Exception as e:
            print(e)
            await message.channel.send(
                "You don't own this stock, {}.".format(str(message.author))
            )
        ...
        try:
            ...
            if len(portfolio) == 0:
                return await message.channel.send(
                    "Your portfolio is empty, {}".format(str(message.author))
                )
            ...
            print(e)
            await message.channel.send(
                "{}'s portfolio could not be found.", format(str(message.author))
            )
